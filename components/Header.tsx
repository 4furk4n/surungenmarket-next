...
export default async function Header() {
  ...
  try {
    ...
    const { data } = await sb.from("site_assets").select("storage_path").eq("key", "logo").maybeSingle();
    logoUrl = siteAssetUrl((data as any)?.storage_path);
  } catch {}
    ...
    <header>
      <div className="topbar">
        <Link href="/" className="logo" title="Ana sayfa" style={{ textDecoration: "none" }}>
          {logoUrl ? (
            <img src={logoUrl} alt="SürüngenMarket" style={{ height: 34, width: "auto", display: "block" }} />
          ) : (
            <>
              <svg width="32" height="32" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="SürüngenMarket leopar gecko logosu">
